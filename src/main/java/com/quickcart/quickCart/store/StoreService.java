...
public class StoreService {
    ...
    public String saveLogoAsWebP(MultipartFile image) {
        try {
            ...
            File outputDir = new File("/application/storeLogo/");
            if (!outputDir.exists()) {
                outputDir.mkdirs();
            }
    ...
    public ResponseEntity<Resource> getLogo(String imageName, boolean download) {
        try {
            Path filePath = Paths.get("/application/storeLogo/" + imageName);
            Resource resource = new FileSystemResource(filePath);

            if (!resource.exists() || !resource.isReadable()) {
                logger.warn("Логотип не найден: {}", imageName);
            ...
            logger.info("Логотип получен: {}", filePath.toAbsolutePath());
            return ResponseEntity.ok()
                    .header(HttpHeaders.CONTENT_DISPOSITION, contentDisposition)
                    .contentType(mediaType)
                    .body(resource);
        } catch (Exception e) {
            logger.error("Ошибка при получении логотипа: {}", e.getMessage(), e);
