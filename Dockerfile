...
WORKDIR /application
COPY --from=build /app/target/*.jar quickCart-0.0.1-SNAPSHOT.jar
RUN java -Djarmode=tools -jar quickCart-0.0.1-SNAPSHOT.jar extract --layers --destination extracted
...
ENTRYPOINT java \
    ...
    $JAVA_CDS_OPTS \
    -jar quickCart-0.0.1-SNAPSHOT.jar
